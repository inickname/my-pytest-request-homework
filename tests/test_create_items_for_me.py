...
from src.enums.enums import Url, Headers
...
class TestItems:
    ...
    def test_create_item(self, item_data, auth_session):
        item_data = item_data()
        response = auth_session.post(self.endpoint, json=item_data.model_dump())
        assert response.status_code in (200, 201), f"Response: {response.status_code}, {response.text}"

        # data = response.json()
        data = validate_response(response, ItemResponseModel, 200, item_data.model_dump())
    ...
    def test_create_item_without_token(self, item_data):
        item_data = item_data()
        response = requests.post(self.endpoint, json=item_data.model_dump(), headers=API_HEADERS)
    ...
    def test_create_item_with_data_too_long(self, auth_session, data_too_long):
        ...
        response = auth_session.post(self.endpoint, json=data_too_long.model_dump())
        assert response.status_code == 422, f"Response: {response.status_code}, {response.text}"
    ...
    def test_update_item(self, item_data, auth_session):
        item_data = item_data()
        response = auth_session.put(f"{self.endpoint}{self.created_item_id}", json=item_data.model_dump())
        assert response.status_code == 200, f"Response: {response.status_code}, {response.text}"
    ...
    def test_update_item_with_data_too_long(self, auth_session, data_too_long):
        data_too_long = data_too_long()
        response = auth_session.put(f"{self.endpoint}{self.created_item_id}", json=data_too_long.model_dump())
        assert response.status_code == 422, f"Response: {response.status_code}, {response.text}"

    def test_update_non_existent_item(self, auth_session, item_data):
        item_data = item_data()
        response = auth_session.put(f"{self.endpoint}non-existent-id", json=item_data.model_dump())
        assert response.status_code == 422, f"Response: {response.status_code}, {response.text}"
